...
{
    public class BrushingAndLinking2 : MonoBehaviour {
        ...
        [SerializeField]
        public List<VisualizationHandler> brushingVisualisations;
        [SerializeField]
        ...
        public void InitialiseBuffersAndTextures(int dataCount)
        {
            ...
            computeShader.SetBuffer(kernelComputeBrushTexture, "brushedIndicesBuffer", brushedIndicesBuffer);
        ...
        public void UpdateComputeBuffers(VisualizationHandler visualisation)
        {
            dataBuffer.SetData(visualisation.NeuronsGo.GetComponent<MeshFilter>().mesh.vertices);
            computeShader.SetBuffer(kernelComputeBrushTexture, "dataBuffer", dataBuffer);
        ...
        public void Update()
        {
            if (isBrushing && brushingVisualisations.Count > 0 && input1 != null && input2 != null)
            {
                ...
                {
                    // TODO: When we get clusters, the neuron count will change
                    InitialiseBuffersAndTextures(brushingVisualisations[0].neurons.Count);
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public HashSet<int> GetBrushedIndices()
        {

            //UpdateBrushedIndices();
            HashSet<int> indicesBrushed = new HashSet<int>();
        ...
        public void SetBrushedIndices(HashSet<int> brushedIndices, bool brushingNotErasing)
        {
            if (!hasInitialised)
            {
                InitialiseBuffersAndTextures(brushingVisualisations[0].neurons.Count);
            ...
            int brushValue = brushingNotErasing ? 1 : -1; //TODO Was 1 or -1
            for (int i = 0; i < this.brushedIndices.Count; i++)
            {
                this.brushedIndices[i] = brushedIndices.Contains(i) ? brushValue : this.brushedIndices[i];
            }
            brushedIndicesBuffer.SetData(this.brushedIndices);
            //computeShader.SetBuffer(kernelComputeBrushedIndices, "brushedIndicesBuffer", brushedIndicesBuffer);
            UpdateBrushTexture();
            /*UpdateBrushedIndices();*/
        }
            ...
            foreach (var vis in brushingVisualisations)
            {
                ...
                switch (BRUSH_TYPE)
                {
                    case BrushType.SPHERE:
                        ...
                        computeShader.SetFloat("visScale", vis.transform.localScale.x);
                        break;
                    case BrushType.BOX:
                        projectedPointer1 = vis.transform.InverseTransformPoint(input1.transform.position);
                ...
                // TODO: Maybe these dummyvalues do not work!!
                ...
                computeShader.SetFloat("_MinX", -9999);
                computeShader.SetFloat("_MaxX", 9999);
                computeShader.SetFloat("_MinY", -9999);
                computeShader.SetFloat("_MaxY", 9999);
                computeShader.SetFloat("_MinZ", -9999);
                computeShader.SetFloat("_MaxZ", 9999);
                ...
                bool somethingSelected = brushedIndices.Contains(1);
                foreach (var visHandler in brushingVisualisations)
                {
                    visHandler.NeuronsGo.GetComponent<MeshRenderer>().sharedMaterial.SetTexture("_BrushedTexture", brushedIndicesTexture);
                    visHandler.NeuronsGo.GetComponent<MeshRenderer>().sharedMaterial.SetFloat("_DataWidth", texSize);
                    visHandler.NeuronsGo.GetComponent<MeshRenderer>().sharedMaterial.SetFloat("_DataHeight", texSize);
                    visHandler.NeuronsGo.GetComponent<MeshRenderer>().sharedMaterial.SetFloat("_ShowBrush", Convert.ToSingle(showBrush));
                    visHandler.NeuronsGo.GetComponent<MeshRenderer>().sharedMaterial.SetColor("_BrushColor", brushColor);
                    visHandler.NeuronsGo.GetComponent<MeshRenderer>().sharedMaterial.SetFloat("_SomethingSelected", somethingSelected ? 1 : 0);
                }
