	...
	SubShader
	{
		Pass
		{
			...
			CGPROGRAM
                ...
                float _DataWidth;
                float _DataHeight;
				float _SomethingSelected; //If something has been brushed
				...
				v2g VS_Main(app_data v)
				{
					...
					float3 vertex = lerp(v.normal, v.vertex, Tween);
					...
					float4 normalisedPosition = float4(
                        normaliseValue(vertex.x, MinNormX, MaxNormX, 0,1),
                        normaliseValue(vertex.y, MinNormY, MaxNormY, 0,1),
                        normaliseValue(vertex.z, MinNormZ, MaxNormZ, 0,1),
                        v.vertex.w);
				...
				void GS_Main(point v2g p[1], inout TriangleStream<g2f> triStream)
				{
					...
					// Access instanced variables
                    float Size = UNITY_ACCESS_INSTANCED_PROP(Props, _Size); //temp update to make sizes smaller. Might change this to 0, so size is entirely controlled by us 
					...
					float halfS = 0.025f * (Size + p[0].size + (dist * sizeFactor));  //updated for size encoding
					...
					o.color = (o.isBrushed && _SomethingSelected)?  UNITY_ACCESS_INSTANCED_PROP(Props, _BrushColor) : p[0].color;
					//o.color = p[0].color;
					//o.color.a = (!o.isBrushed && _SomethingSelected)?0.5f : 1;
					o.normal = p[0].normal;
