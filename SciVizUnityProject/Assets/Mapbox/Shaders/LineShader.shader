		...
		Pass
		{
			...
			CGPROGRAM
				...
				void geom(line v2g points[2], inout TriangleStream<g2f> triStream)
				{
					...
					float width = points[0].size + normaliseValue(points[0].normal.y, 0.0, 1.0, MinSize, MaxSize);
					...
					v[0].tex0 = float2(1,0);
					v[0].color = points[1].color; //changed this to points[1] so that the whole line has the same color
					v[0].isBrushed = points[0].isBrushed;// || points[1].isBrushed;

					v[1].vertex = float4(p0_ex + normal_scaled, 1) * w0;
					v[1].tex0 = float2(0,0);
					v[1].color = points[1].color;  //changed this to points[1] so that the whole line has the same color
